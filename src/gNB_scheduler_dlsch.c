...
#include "NR_MAC_gNB/nr_mcs_cap.h"  // already included in mac_sm_ric.c; include in scheduler file too
 ...
 * MANUAL MCS CAP (DL)
 * ======================= */
// Set this value for each experiment:
//   28 = no effective cap (your current behavior)
//   24, 20, 16, 12, ... for lower MCS tests
     const int manual_mcs_cap = 28;   // <-- CHANGE THIS VALUE BETWEEN RUNS
     const int max_mcs_cfg = min(sched_ctrl->dl_max_mcs, max_mcs_table);
     
/* Final cap applied */
     //int max_mcs = min(manual_mcs_cap, max_mcs_cfg);
     int max_mcs = max_mcs_cfg;
     if (nr_get_dl_mcs_cap_enable()) {
     max_mcs = min(max_mcs, (int)nr_get_dl_mcs_cap_value());
}

LOG_I(NR_MAC,
      "[MCS-CAP] xapp_en=%d xapp_cap=%d final_max_mcs=%d (manual=%d cfg=%d table=%d)\n",
      nr_get_dl_mcs_cap_enable(),
      nr_get_dl_mcs_cap_value(),
      max_mcs,
      manual_mcs_cap,
      max_mcs_cfg,
      max_mcs_table);
  ...
  while (iterator->UE != NULL) {
    ...
    * allocation after CCE alloc fail would be more complex) */
    ...
    nr_find_nb_rb(sched_pdsch.Qm,
                  ...
                  &sched_pdsch.tb_size,
                  &sched_pdsch.rbSize);
   /* ── E2SM-RC Style 2: enforce per-UE PRB quota from xApp ── */
    if (iterator->UE->UE_sched_ctrl.rc_prb_quota_set) {
      const uint16_t bwp_prbs = bwp_info.bwpSize;
      const uint16_t rc_max_rb_raw = (bwp_prbs * iterator->UE->UE_sched_ctrl.rc_max_prb_pct) / 100;
      const uint16_t max_rb = rc_max_rb_raw > (uint16_t)min_rbSize ? rc_max_rb_raw : (uint16_t)min_rbSize;
      const uint16_t min_rb   = (bwp_prbs * iterator->UE->UE_sched_ctrl.rc_min_prb_pct) / 100;

      if (sched_pdsch.rbSize > max_rb) {
        LOG_I(NR_MAC,
              "[RC-QUOTA] RNTI 0x%04x: capped rbSize %u → %u (max=%u%%)\n",
              rnti, sched_pdsch.rbSize, max_rb,
              iterator->UE->UE_sched_ctrl.rc_max_prb_pct);
        sched_pdsch.rbSize = max_rb;
        /* Recompute TBS to match the capped rbSize */
      ...
      if (sched_pdsch.rbSize < min_rb)
        sched_pdsch.rbSize = min_rb;
    }
    /* ─────────────────────────────────────────────────────── */                  
     ...
     // >>> MCS DEBUG PRINT <<<
    LOG_I(NR_MAC,
          "[MCS_TRACE] %4d.%2d UE %04x beam %d rbStart %3d rbSize %3d  MCS %2d  Qm %d  R %d  TBS %5d\n",
          frame,
          slot,
          rnti,
          beam.idx,
...
void post_process_dlsch(gNB_MAC_INST *nr_mac, post_process_pdsch_t *pdsch, NR_UE_info_t *UE, NR_sched_pdsch_t *sched_pdsch)
{
  ...
  if (harq->round != 0) { /* retransmission */
   ...
   /* ── E2SM-RC Style 2: enforce per-UE PRB quota from xApp ── */
          ...
          /*
           * NRRIV2BW decodes the RIV-encoded BWP bandwidth into
           * an actual PRB count. Using the raw field directly gives
           * a wrong number — this macro is mandatory.
           */
