...
static void fill_rc_control(ran_func_def_ctrl_t* ctrl)
{
  ...
  ctrl_style->ran_param_ctrl_out = NULL;
    /* ════════════════════════════════════════════════════════
   * STYLE 2 — Radio Resource Allocation Control  (NEW)
   * ════════════════════════════════════════════════════════ */
  seq_ctrl_style_t* ctrl_style2 = &ctrl->seq_ctrl_style[1];

  ctrl_style2->style_type = 2;

  const char s2_name[] = "Radio Resource Allocation Control";
  ctrl_style2->name = cp_str_to_ba(s2_name);
  ...
  /* One control action: Action 6 — PRB Quota Setting */
  ctrl_style2->sz_seq_ctrl_act = 1;
  ctrl_style2->seq_ctrl_act    = calloc(1, sizeof(seq_ctrl_act_2_t));
  assert(ctrl_style2->seq_ctrl_act != NULL && "Memory exhausted");

  seq_ctrl_act_2_t* act6 = &ctrl_style2->seq_ctrl_act[0];
  act6->id = 6;
  const char act6_name[] = "PRB Quota Setting";
  act6->name = cp_str_to_ba(act6_name);

  /* Four INTEGER RAN parameters */
  act6->sz_seq_assoc_ran_param = 4;
  act6->assoc_ran_param = calloc(4, sizeof(seq_ran_param_3_t));
  assert(act6->assoc_ran_param != NULL && "Memory exhausted");

  /* Param 1 — Min PRB Policy Ratio */
  act6->assoc_ran_param[0].id = RC_S2_ACT6_PARAM_MIN_PRB;
  const char p1_name[] = "Min PRB Policy Ratio";
  act6->assoc_ran_param[0].name = cp_str_to_ba(p1_name);
  act6->assoc_ran_param[0].def = NULL;

  /* Param 2 — Max PRB Policy Ratio */
  act6->assoc_ran_param[1].id = RC_S2_ACT6_PARAM_MAX_PRB;
  const char p2_name[] = "Max PRB Policy Ratio";
  act6->assoc_ran_param[1].name = cp_str_to_ba(p2_name);
  act6->assoc_ran_param[1].def = NULL;

  /* Param 3 — Dedicated PRB Policy Ratio */
  act6->assoc_ran_param[2].id = RC_S2_ACT6_PARAM_DED_PRB;
  const char p3_name[] = "Dedicated PRB Policy Ratio";
  act6->assoc_ran_param[2].name = cp_str_to_ba(p3_name);
  act6->assoc_ran_param[2].def = NULL;

  /* Param 4 — Target C-RNTI */
  act6->assoc_ran_param[3].id = RC_S2_ACT6_PARAM_RNTI;
  const char p4_name[] = "Target C-RNTI";
  act6->assoc_ran_param[3].name = cp_str_to_ba(p4_name);
  act6->assoc_ran_param[3].def = NULL;
  ...
  sm_ag_if_ans_t write_ctrl_rc_sm(void const* data)
{
  ...
  /* ──────────────────────────────────────────────────────────
   * ACTION 6 — Style 2: PRB Quota Setting  (NEW)
   * ────────────────────────────────────────────────────────── */
   } else if (style == 2 && act_id == 6) {
    printf("[RC CTRL] Style 2 / Act 6 — PRB Quota Setting\n");

    ue_id_e2sm_t ue_id = ctrl->hdr.frmt_1.ue_id;
    uint16_t rnti = 0;
    uint16_t rnti_from_hdr = 0;
    uint16_t body_rnti = 0;
    bool hdr_rnti_valid = false;

    printf("[RC CTRL] UE ID type=%d\n", ue_id.type);

    if (ue_id.type == GNB_UE_ID_E2SM) {
      if (ue_id.gnb.ran_ue_id != NULL) {
        rnti_from_hdr = (uint16_t)(*ue_id.gnb.ran_ue_id);
        hdr_rnti_valid = true;
        printf("[RC CTRL] Header UE ID: ran_ue_id=0x%lx -> RNTI=0x%04x\n",
               (unsigned long)(*ue_id.gnb.ran_ue_id), rnti_from_hdr);
      } else {
        printf("[RC CTRL] Header UE ID: ran_ue_id is NULL — will rely on body param RNTI\n");
      }
    } else {
      printf("[RC CTRL] UE ID type %d not handled for Style 2 — dropping\n",
             ue_id.type);
      return ans;
    }

    const seq_ran_param_t* ran_param = ctrl->msg.frmt_1.ran_param;
    const size_t n_param = ctrl->msg.frmt_1.sz_ran_param;

    uint8_t min_prb_pct = 0;
    uint8_t max_prb_pct = 100;
    bool got_min = false, got_max = false;

    for (size_t i = 0; i < n_param; i++) {
      const uint32_t pid = ran_param[i].ran_param_id;

      if (ran_param[i].ran_param_val.type
          != ELEMENT_KEY_FLAG_FALSE_RAN_PARAMETER_VAL_TYPE) {
        printf("[RC CTRL] Param %u: unexpected val type %d — skipped\n",
               pid, ran_param[i].ran_param_val.type);
        continue;
      }

      int64_t raw = ran_param[i].ran_param_val.flag_false->int_ran;

      if (pid != RC_S2_ACT6_PARAM_RNTI && (raw < 0 || raw > 100)) {
        printf("[RC CTRL] Param %u: value %ld out of range [0,100] — skipped\n",
               pid, raw);
        continue;
      }

      switch (pid) {
        case RC_S2_ACT6_PARAM_MIN_PRB:
          min_prb_pct = (uint8_t)raw;
          got_min = true;
          printf("[RC CTRL] Min PRB ratio = %u%%\n", min_prb_pct);
          break;

        case RC_S2_ACT6_PARAM_MAX_PRB:
          max_prb_pct = (uint8_t)raw;
          got_max = true;
          printf("[RC CTRL] Max PRB ratio = %u%%\n", max_prb_pct);
          break;

        case RC_S2_ACT6_PARAM_DED_PRB:
          min_prb_pct = max_prb_pct = (uint8_t)raw;
          got_min = got_max = true;
          printf("[RC CTRL] Dedicated PRB ratio = %u%%\n", min_prb_pct);
          break;

        case RC_S2_ACT6_PARAM_RNTI:
          if (raw < 0 || raw > 65535) {
            printf("[RC CTRL] RNTI value %ld out of range [0,65535] — skipped\n",
                   raw);
            break;
          }
          body_rnti = (uint16_t)raw;
          printf("[RC CTRL] Body RNTI = 0x%04x\n", body_rnti);
          break;

        default:
          printf("[RC CTRL] Unknown param ID %u — skipped\n", pid);
          break;
      }
    }

    if (body_rnti != 0) {
      printf("[RC CTRL] Using explicit body RNTI 0x%04x instead of header RNTI 0x%04x\n",
             body_rnti, rnti_from_hdr);
      rnti = body_rnti;
    } else if (hdr_rnti_valid) {
      rnti = rnti_from_hdr;
      printf("[RC CTRL] Using header RNTI 0x%04x\n", rnti);
    } else {
      printf("[RC CTRL] ERROR: no valid RNTI — dropping\n");
      return ans;
    }

    if (!got_min && !got_max) {
      printf("[RC CTRL] No valid PRB params in message — dropping\n");
      return ans;
    }

    int rc = e2sm_rc_apply_prb_quota(rnti, min_prb_pct, max_prb_pct);

    if (rc != 0) {
      printf("[RC CTRL] PRB quota apply FAILED for RNTI 0x%04x\n", rnti);
    } else {
      printf("[RC CTRL] PRB quota applied OK for RNTI 0x%04x\n", rnti);
    } 
   } else {
    printf("[RC CTRL] Unknown style=%u action ID=%u — ignoring\n", style, act_id);
  }
